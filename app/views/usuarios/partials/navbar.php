<nav class="hidden md:flex md:px-16 bg-primaryDark text-lg text-white tracking-wider font-irish">
	...
	<div class="hidden md:flex w-full items-center space-x-4 justify-end">
		<li class="list-none">
			<a class="p-3 <?= ($data['page'] == 'biblioteca') ? 'is-active' : 'is-inactive'; ?>" href="<?php echo URLROOT . '/' . $data['controller'] . '/biblioteca/' . $data['project_name'] ?>"><i class="fas fa-book text-xl mr-3"></i>MI BIBLIOTECA</a>
		</li>
...
<div class="md:hidden">
  <div id="mobile-menu" class="hidden bg-primary divide-y flex-col items-center text-xl text-white left-0 right-0 tracking-wider font-irish">
    ...
    <a href="<?php echo URLROOT . '/' . $data['controller'] . '/biblioteca/' . $data['project_name'] ?>" class="w-full text-center py-4 hover:bg-ctaLight hover:text-dark">
    	<i class="fas fa-book text-xl mr-3"></i>Mi biblioteca
    </a>
