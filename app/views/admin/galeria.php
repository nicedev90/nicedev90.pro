...
<?php if (isset($data['chapter'])) : ?>
	...
	<div class="flex flex-col md:flex-row p-2 md:px-16 my-0 mx-auto ">
		...
		<!-- columna IZQUIERDA -->
		<div id="col-izq" class="bg-neutral p-3 my-4 md:rounded-none rounded-lg space-y-4 md:w-3/5">

			<div class="flex justify-between px-2 md:px-5 items-center">
				<div id="chapter" class="space-x-10 text-4xl md:text-6xl font-bold" data-comic="<?php echo $data['comic']->nombre ?>" data-price="<?php echo $data['chapter']->precio ?>" data-chapter="<?php echo $data['chapter']->titulo ?>" data-quant="1">
					<span class="text-white"><?php echo $data['chapter']->titulo ?></span>
					<?php if (strtoupper($data['chapter']->titulo) == 'PAID') : ?>
						<span class="text-cta"> <i class="fas fa-star ml-4"></i></span>
					<?php else : ?>
			...
			<?php if (strtoupper($data['chapter']->titulo) == 'PAID') : ?>
			<div class="flex justify-between px-2 md:px-5 items-center">
				<a href="<?php echo URLROOT ?>/pages/login" class="w-1/2 text-center p-3 cursor-pointer mx-auto rounded-full font-bold text-white bg-primary hover:bg-primaryDark">Ver en mi Biblioteca </a>
			</div>
			<?php endif; ?>
			...
			<div class="flex justify-between px-2 pt-8 md:px-5 items-center">
				<div id="paypal_checkout" class="hidden w-full flex flex-col space-y-4 p-4 bg-white rounded-xl">
					<div class="flex justify-between items-center ">
						<p> Puedes realizar tu compra :</p>
						<button id="close_paypal" class="text-2xl px-2"><i class="fas fa-circle-xmark"></i></button>
					</div>
			...
			<?php if (strtoupper($data['chapter']->estado) == 'PUBLICADO') : ?>
				<div class="relative flex justify-between px-2 pt-8 md:px-5 items-center">
		...
		<!-- columna DERECHA -->
		<div id="col-der" class="hidden md:block p-2 md:p-4 md:w-full">
